...
create table atendimento (cod_atendimento serial not null, descrever_sessao varchar(255), objetivo_sessao varchar(255), cod_consulta int8 not null, primary key (cod_atendimento));
create table categoria_lancamento (cod_lancamento serial not null, nm_categoria_lanc varchar(255), primary key (cod_lancamento));
create table cidade (cod_cidade serial not null, cod_estado int8 not null, nm_cidade varchar(255) not null, primary key (cod_cidade));
create table consulta (cod_consulta serial not null, class_name varchar(255), convenio_enum varchar(255), data_hora timestamp, local_atendimento varchar(255), observacao varchar(255), procedimento_enum varchar(255), start timestamp, status_consulta_enum varchar(255), title varchar(255), clinica int8, paciente int8, profissional int8, primary key (cod_consulta));
create table estado (cod_estado serial not null, nm_estado varchar(255) not null, sigla varchar(255) not null, primary key (cod_estado));
create table forma_pagamento (id serial not null, data_atualizacao timestamp, descricao varchar(255) not null, primary key (id));
create table foto_perfil (cod_foto_perfil serial not null, content_type varchar(255), descricao varchar(255), nome_arquivo varchar(255), tamanho int8, primary key (cod_foto_perfil));
create table grupo (cod_grupo serial not null, nm_grupo varchar(255), primary key (cod_grupo));
create table grupo_permissao (cod_grupo serial not null, cod_permissao int8 not null);
create table lancamento (cod_lancamento serial not null, clinica_id int8, descricao varchar(255), dt_lancamento timestamp, profissional_id int8, valor_total numeric(19, 2), categoria_cod_lancamento int8, consulta_cod_consulta int8, forma_pagamento_id int8, primary key (cod_lancamento));
create table permissao (cod_permissao serial not null, desc_permissao varchar(255) not null, nm_permissao varchar(255) not null, primary key (cod_permissao));
create table usuario (tp varchar(31) not null, cod_usuario serial not null, ativado varchar(255), endereco_bairro varchar(255), celular varchar(255), endereco_cep varchar(255), endereco_complemento varchar(255), cpf varchar(255), dt_cad_usuario timestamp not null, dt_nascimento timestamp, email varchar(255), endereco_logradouro varchar(255), nm_usuario varchar(255), endereco_numero varchar(255), senha varchar(255), sexo varchar(255), telefone varchar(255), clinica_id int8, cpf_responsavel varchar(255), nome_mae varchar(255), nome_pai varchar(255), profissional_id int8, conselho varchar(255), formacao_academica varchar(255), registro_conselho varchar(255), endereco_cidade_id int8, foto_perfil_cod_foto_perfil int8, primary key (cod_usuario));
create table usuario_grupo (cod_usuario int8 not null, cod_grupo int8 not null);
create table verificar_token (id serial not null, expiry_date timestamp, token varchar(255), cod_usuario int8 not null, primary key (id));
